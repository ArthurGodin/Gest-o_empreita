    ...
    <div className="space-y-3">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-lg border bg-card shadow-[0_1px_2px_rgba(15,23,42,0.035)]">
          <div className="hidden grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)_minmax(0,0.7fr)_auto] gap-4 border-b bg-muted/60 px-4 py-2.5 text-xs font-semibold text-muted-foreground md:grid">
            <span>Cliente</span>
            <span>Telefone</span>
            <span>Local</span>
...
function CustomerRow({ customer }: { customer: Customer }) {
  ...
  return (
    <li className="group relative grid min-h-[104px] grid-cols-[minmax(0,1fr)_auto] gap-x-3 gap-y-2 px-4 py-3.5 transition-colors hover:bg-accent/60 focus-within:bg-accent/60 md:min-h-16 md:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)_minmax(0,0.7fr)_auto] md:items-center md:gap-4 md:py-3">
      <Link
      ...
      <span className="pointer-events-none flex min-w-0 items-center gap-2 text-sm text-muted-foreground">
